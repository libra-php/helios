...
class Controller
{
            ...
            foreach ($ruleset as $rule) {
                ...
                if ($result) {
                    ...
                    if (
                        ...
                        $this->addRequestError(
                            $key,
                            sprintf($this->error_messages[$rule], $rule_arg)
                        );
                    } elseif (isset($this->error_messages[$key])) {
                        // Look for other error message entry (key only)
                        $this->addRequestError(
                            $key,
                            sprintf($this->error_messages[$key], $rule_arg)
                        );
                    } else {
                        $this->addRequestError(
                            "_",
    ...
    public function validate(
        ...
        return match (strtolower($rule)) {
            ...
            "unique" => !db()->fetch(
                "SELECT 1 FROM $rule_arg WHERE $key = ?",
                $value
            ),
            ...
            "date_format" => date_create_from_format($rule_arg, $value) !==
                false,
            ...
            default => throw new InvalidArgumentException(
                "Validation rule '$rule' is not supported."
            ),
        };
    }
    ...
    {
        if (trim($message) != "") {
            $this->request_errors[$key][] = $message;
        }
    }
