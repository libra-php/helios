...
class Model implements IModel
{
    ...
    /**
     * Fetch a single model from table
     */
    public static function fetch(array $columns = ['*'], array $where = [], ...$replacements): Model|bool
    {
        $class = get_called_class();
        $model = new $class();
        $key_column = $model->key_column;
        if (!in_array($key_column, $columns)) {
            $columns[] = $key_column;
        }

        $result = QueryBuilder::select($columns)
            ->from($model->table_name)
            ->where($where, ...$replacements)
            ->execute()
            ->fetch();
        if ($result) {
            $key_column = $model->key_column;
    ...
    /**
     * Fetch a single column from a model
     */
    public static function var(string $column, array $where = [], ...$replacements): string|bool
    {
        $class = get_called_class();
        $model = new $class();

        $result = QueryBuilder::select([$column])
            ->from($model->table_name)
            ->where($where, ...$replacements)
            ->execute()
    ...
    /**
     * Fetch all models from table 
     */
    public static function fetchAll(array $columns = ['*'], array $where = [], ...$replacements): array|bool
    {
        $class = get_called_class();
        $model = new $class();
        $results = QueryBuilder::select($columns)
            ->from($model->table_name)
            ->where($where, ...$replacements)
            ->execute()
            ->fetch();
        ...
        if ($results) {
            foreach ($results as $result) {
                $key_column = $model->key_column;
                $models[] = new $model($result->$key_column);
            }
            return $models;
        }
        return false;
    ...
    /**
     * Search for records using QueryBuilder
     */
